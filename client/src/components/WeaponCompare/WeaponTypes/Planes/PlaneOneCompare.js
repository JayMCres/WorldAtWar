...
class PlaneOneCompare extends Component {
  state = {
    activeItem: "profile"
  };
...
const mapStateToProps = state => ({
  profile: [state.weapon.planeOne].map(item => {
    return item[1];
  }),

  combat: [state.weapon.planeOne].map(item => {
    return item[1];
  }),
  bombs: [state.weapon.planeOne].map((item, index) => item[3]),
  guns: [state.weapon.planeOne].map(item => {
    return item[6];
  }),
  allWeapons: [state.weapon.planeOne[6]]
    .concat([state.weapon.planeOne[3]])
    .map(items => {
      return { ...items };
    }),
  planeObj: [state.weapon.planeOne[3]].map(item => item)
});

export default connect(mapStateToProps)(PlaneOneCompare);
